...
class MultiFrameworksWorkflowConfig(FunctionBaseConfig, name="multi_frameworks_llm_router"):
    ...
    data_dir: str = "/home/coder/dev/ai-query-engine/examples/multi_frameworks_llm_router/data/"
...
async def multi_frameworks_llm_router_workflow(config: MultiFrameworksWorkflowConfig, builder: Builder):
    ...
    'Retrieve' - any question related to the topic of AgentIQ or its workflows, especially concerning the particular workflow called multi_frameworks_llm_router which show case using multiple frameworks such as langchain, llama-index ..etc
    ...
    async def workers(state: AgentState):
        ...
        if "retrieve" in worker_choice.lower():
            ...
            # inputs = {"inputs": query}
            completion = (await llm_router_tool.ainvoke(query))
            ...
            output = ("Apologies, I am not sure what to say, I can answer general questions retrieve info this "
                      "multi_frameworks_llm_router workflow and answer light coding questions, but nothing more.")
    ...
    try:
        ...
        logger.debug("Cleaning up multi_frameworks_llm_router workflow.")
